        ...
        btn = [
            [
                InlineKeyboardButton(
                    text=f"〶 {stylish_size(file.file_size)} {file.file_name}", callback_data=f'files#{file.file_id}'
                ),
            ]
            for file in files
        ]
...
async def auto_filter(client, msg, spoll=False):
    ...
    if settings["button"]:
        btn = [
            [
                InlineKeyboardButton(
                    text=f"📁 {stylish_size(file.file_size)} 🔺 {file.file_name}", callback_data=f'{pre}#{req}#{file.file_id}'
                ),
            ]
            for file in files
        ]
